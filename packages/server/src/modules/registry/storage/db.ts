...
const REGISTRY_MIGRATIONS_DIR = path.join(__dirname, '../../../db/migrations/registry');
...
export class RegistryDb {
  ...
  initialize(): void {
    ...
    runMigrations(this.db, REGISTRY_MIGRATIONS_DIR);
  }
  ...
  schemaVersion(): number {
    ...
    if (!row) throw new Error('schema_versions row missing');
    return row.version;
  }
